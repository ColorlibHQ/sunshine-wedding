/*! ColorlibUI 3.0.0 — core, owl, magnific, isotope, slicknav, scrollup, ajaxchimp, countdown, owl markup 2.2. Built for this theme from core + the modules it uses. */
/**
...
/* ColorlibUI module: slicknav — replaces SlickNav 1.0.10. The themes clone their
 * desktop menu into a mobile toggle menu and style the exact DOM SlickNav builds
 * (.slicknav_menu > .slicknav_btn + ul.slicknav_nav with .slicknav_parent /
 * .slicknav_item / .slicknav_arrow rows), so this builds the same elements,
 * classes, roles, aria-hidden and tabindex values, toggles the same classes and
 * slides open/closed with the same jQuery-style height animation. Callbacks
 * receive Elements instead of jQuery objects. */
(function () {
  'use strict';
  var UI = window.ColorlibUI;
  if (!UI) return;
  ...
  var P = 'slicknav';
  var KEY = { DOWN: 40, ENTER: 13, ESCAPE: 27, LEFT: 37, RIGHT: 39, SPACE: 32, UP: 38 };
  function noop() {}
  var DEFAULTS = {
    label: 'MENU', duplicate: true, duration: 200, easingOpen: 'swing', easingClose: 'swing',
    closedSymbol: '&#9658;', openedSymbol: '&#9660;', prependTo: 'body', appendTo: '',
    parentTag: 'a', closeOnClick: false, allowParentLinks: false, nestedParentLinks: true,
    showChildren: false, removeIds: true, removeClasses: false, removeStyles: false, brand: '',
    animations: 'jquery', init: noop, beforeOpen: noop, beforeClose: noop, afterOpen: noop, afterClose: noop
  };
  ...
  function children(el, tag) {
    return Array.prototype.filter.call(el.children, function (c) { return c.tagName === tag; });
  }
  ...
  function setAll(list, attr, value) { list.forEach(function (el) { el.setAttribute(attr, value); }); }

  /*
   * jQuery's slideDown/slideUp: height, vertical padding and margin tween
   * between 0 and their natural values with overflow hidden, then the inline
   * values are cleared again so only `display: none` (when closed) remains.
   * A new slide first jumps the running one to its end, like .stop(true, true).
   */
  function slide(el, show, ms, easing, done) {
    if (el._clSlide) el._clSlide.finish();
    var hidden = isHidden(el);
    if (show === !hidden) {
      // Already in the requested state: jQuery still completes asynchronously.
      if (ms) setTimeout(done, 0); else done();
      return;
    }
    if (show) {
      el.style.display = '';
    ...
    var cs = getComputedStyle(el), full = {}, i;
    for (i = 0; i < SLIDE_PROPS.length; i++) full[SLIDE_PROPS[i]] = parseFloat(cs[SLIDE_PROPS[i]]) || 0;
    function restore() {
      for (var j = 0; j < SLIDE_PROPS.length; j++) el.style[SLIDE_PROPS[j]] = '';
      el.style.overflow = '';
      if (!show) el.style.display = 'none';
    }
    if (!ms || UI.reducedMotion) { restore(); done(); return; }
    var ease = EASING[easing] || EASING.swing;
    var start = null, frame, finished = false;
    el.style.overflow = 'hidden';
    function apply(p) {
      var e = show ? ease(p) : 1 - ease(p);
      for (var j = 0; j < SLIDE_PROPS.length; j++) el.style[SLIDE_PROPS[j]] = (full[SLIDE_PROPS[j]] * e) + 'px';
    }
    function finish() {
      if (finished) return;
      finished = true;
    ...
    apply(0);
    el._clSlide = { finish: finish };
    frame = requestAnimationFrame(step);
  }

  /** Wrap nodes (in document order, same parent) in `wrapper`, like jQuery's wrapAll. */
  ...
  function SlickNav(source, options) {
    var self = this;
    var s = self.settings = UI.extend({}, DEFAULTS, options);
    if (!s.duplicate && !(options && Object.prototype.hasOwnProperty.call(options, 'removeIds'))) s.removeIds = false;
    self.element = source;

    var nav = self.mobileNav = s.duplicate ? source.cloneNode(true) : source;
    function strip(attr) {
      nav.removeAttribute(attr);
      Array.prototype.forEach.call(nav.querySelectorAll('[' + attr + ']'), function (el) { el.removeAttribute(attr); });
    }
    if (s.removeIds) strip('id');
    if (s.removeClasses) strip('class');
    if (s.removeStyles) strip('style');

    // parentTag may carry attributes ('a href="#"'), so markup is built as a string like the original.
    var tag = s.parentTag === 'a' ? 'a href="#"' : s.parentTag;
    var tagName = String(tag).split(/\s/)[0];
    function build(html) {
      var t = document.createElement('template');
      t.innerHTML = html;
      return t.content.firstElementChild;
    }
    var itemHTML = '<' + tag + ' role="menuitem" aria-haspopup="true" tabindex="-1" class="' + P + '_item"></' + tagName + '>';

    nav.setAttribute('class', P + '_nav');
    var menu = self.menu = document.createElement('div');
    menu.className = P + '_menu';
    if (s.brand !== '') menu.appendChild(build('<div class="' + P + '_brand">' + s.brand + '</div>'));
    var btn = self.btn = build(
      '<' + tag + ' aria-haspopup="true" role="button" tabindex="0" class="' + P + '_btn ' + P + '_collapsed">' +
      '<span class="' + P + '_menutxt">' + s.label + '</span>' +
      '<span class="' + P + '_icon' + (s.label === '' ? ' ' + P + '_no-text' : '') + '">' +
      '<span class="' + P + '_icon-bar"></span><span class="' + P + '_icon-bar"></span><span class="' + P + '_icon-bar"></span>' +
      '</span></' + tagName + '>');
    menu.appendChild(btn);
    // jQuery inserts the live node into the last matched container.
    var hosts = UI.toElements(s.appendTo !== '' ? s.appendTo : s.prependTo);
    var host = hosts[hosts.length - 1];
    if (host) {
      if (s.appendTo !== '') host.appendChild(menu); else host.insertBefore(menu, host.firstChild);
    }
    menu.appendChild(nav);

    var lis = Array.prototype.slice.call(nav.querySelectorAll('li'));
    lis.forEach(function (li) {
      var subs = children(li, 'UL');
      subs.forEach(function (ul) { ul.setAttribute('role', 'menu'); });
      li._clSubs = subs;
      if (subs.length) {
        // Everything before the first submenu becomes the clickable row.
        var nodes = [], hasLink = false;
        for (var n = li.firstChild; n; n = n.nextSibling) {
          if (n.nodeType === 1 && n.tagName === 'UL') break;
          nodes.push(n);
          if (n.nodeType === 1 && n.tagName === 'A') hasLink = true;
        }
        var split = s.allowParentLinks && !s.nestedParentLinks && hasLink;
        if (nodes.length) {
          if (split) wrapAll(nodes, build('<span class="' + P + '_parent-link ' + P + '_row"></span>'));
          else wrapAll(nodes, build(itemHTML)).classList.add(P + '_row');
        }
        li.classList.add(s.showChildren ? P + '_open' : P + '_collapsed');
        li.classList.add(P + '_parent');
        var arrow = build('<span class="' + P + '_arrow">' + (s.showChildren ? s.openedSymbol : s.closedSymbol) + '</span>');
        if (split) { var wrap = build(itemHTML); wrap.appendChild(arrow); arrow = wrap; }
        if (nodes.length) {
          var last = nodes[nodes.length - 1];
          last.parentNode.insertBefore(arrow, last.nextSibling);
        }
      } else if (li.children.length === 0) {
        li.classList.add(P + '_txtnode');
      }
      children(li, 'A').forEach(function (a) {
        a.setAttribute('role', 'menuitem');
        a.addEventListener('click', function (e) {
          var li2 = e.target.parentNode && e.target.parentNode.closest ? e.target.parentNode.closest('li') : null;
          if (s.closeOnClick && !(li2 && li2.classList.contains(P + '_parent'))) btn.click();
        });
      });
      if (s.closeOnClick && s.allowParentLinks) {
        children(li, 'A').forEach(function (a) {
          children(a, 'A').forEach(function (inner) { inner.addEventListener('click', function () { btn.click(); }); });
        });
        Array.prototype.forEach.call(li.querySelectorAll('.' + P + '_parent-link a:not(.' + P + '_item)'), function (a) {
          a.addEventListener('click', function () { btn.click(); });
        });
      }
    });

    lis.forEach(function (li) {
      if (!s.showChildren) li._clSubs.forEach(function (ul) { self._visibilityToggle(ul, null, false, null, true); });
    });
    self._visibilityToggle(nav, null, false, 'init', true);
    nav.setAttribute('role', 'menu');

    // Hide focus rings for mouse users, show them again on keyboard use (as SlickNav did).
    document.addEventListener('mousedown', function () { self._outlines(false); });
    document.addEventListener('keyup', function () { self._outlines(true); });

    btn.addEventListener('click', function (e) { e.preventDefault(); self._menuToggle(); });
    nav.addEventListener('click', function (e) {
      var item = e.target.closest ? e.target.closest('.' + P + '_item') : null;
      if (!item || !nav.contains(item)) return;
      e.preventDefault();
      self._itemClick(item);
    });
    btn.addEventListener('keydown', function (e) {
      switch (e.keyCode) {
        case KEY.ENTER: case KEY.SPACE: case KEY.DOWN:
          e.preventDefault();
          if (!(e.keyCode === KEY.DOWN && btn.classList.contains(P + '_open'))) self._menuToggle();
          var next = btn.nextElementSibling;
          var first = next && next.querySelector('[role="menuitem"]');
          if (first) first.focus();
      }
    });
    nav.addEventListener('keydown', function (e) {
      var t = e.target;
      // Handlers for .slicknav_item run before the generic [role=menuitem] ones, as they were bound first.
      if (t.classList.contains(P + '_item')) {
        if (e.keyCode === KEY.ENTER) { e.preventDefault(); self._itemClick(t); }
        else if (e.keyCode === KEY.RIGHT) {
          e.preventDefault();
          if (t.parentNode.classList.contains(P + '_collapsed')) self._itemClick(t);
          var sub = t.nextElementSibling;
          var f = sub && sub.querySelector('[role="menuitem"]');
          if (f) f.focus();
        }
      }
      if (t.getAttribute('role') !== 'menuitem') return;
      var ul = t.parentNode.parentNode, list, idx;
      function siblings() {
        var out = [];
        Array.prototype.forEach.call(ul.children, function (li) {
          Array.prototype.forEach.call(li.children, function (c) {
            if (c.getAttribute('role') === 'menuitem' && isVisible(c)) out.push(c);
          });
        });
        return out;
      }
      switch (e.keyCode) {
        case KEY.DOWN:
          e.preventDefault();
          list = siblings(); idx = list.indexOf(t);
          if (list.length) list[idx + 1 < list.length ? idx + 1 : 0].focus();
          break;
        case KEY.UP:
          e.preventDefault();
          list = siblings(); idx = list.indexOf(t);
          if (list.length) list[idx - 1 >= 0 ? idx - 1 : list.length - 1].focus();
          break;
        case KEY.LEFT:
          e.preventDefault();
          if (ul.parentNode && ul.parentNode.classList && ul.parentNode.classList.contains(P + '_open')) {
            var prev = ul.previousElementSibling;
            if (prev) { prev.focus(); self._itemClick(prev); }
          } else if (ul.classList.contains(P + '_nav')) {
            self._menuToggle();
            btn.focus();
          }
          break;
    ...
    if (s.allowParentLinks && s.nestedParentLinks) {
      Array.prototype.forEach.call(document.querySelectorAll('.' + P + '_item a'), function (a) {
        a.addEventListener('click', function (e) { e.stopImmediatePropagation(); e.stopPropagation(); });
      });
    }
  }

  SlickNav.prototype._menuToggle = function () {
    var btn = this.btn;
    if (btn.classList.contains(P + '_collapsed')) {
      btn.classList.remove(P + '_collapsed');
      btn.classList.add(P + '_open');
    } else {
      btn.classList.remove(P + '_open');
      btn.classList.add(P + '_collapsed');
    }
    btn.classList.add(P + '_animating');
    this._visibilityToggle(this.mobileNav, btn.parentNode, true, btn);
  };

  SlickNav.prototype._itemClick = function (item) {
    var s = this.settings;
    var arrow = null, ul = item.nextElementSibling, parent = item.parentNode;
    Array.prototype.forEach.call(item.children, function (c) { if (c.classList.contains(P + '_arrow')) arrow = c; });
    if (parent.classList.contains(P + '_parent-link')) {
      parent = parent.parentNode;
      ul = item.parentNode.nextElementSibling;
    }
    if (ul && ul.tagName !== 'UL') ul = null;
    var opening = parent.classList.contains(P + '_collapsed');
    if (arrow) arrow.innerHTML = opening ? s.openedSymbol : s.closedSymbol;
    parent.classList.toggle(P + '_collapsed', !opening);
    parent.classList.toggle(P + '_open', opening);
    parent.classList.add(P + '_animating');
    if (ul) this._visibilityToggle(ul, parent, true, item);
    else parent.classList.remove(P + '_animating');
  };

  /** Links and items that should be tabbable while `ul` is open. */
  function actionItems(ul) {
    var out = [];
    Array.prototype.forEach.call(ul.children, function (li) {
      if (li.tagName !== 'LI') return;
      Array.prototype.forEach.call(li.querySelectorAll('a, .' + P + '_item'), function (el) {
  ...
  SlickNav.prototype._visibilityToggle = function (ul, parent, animate, trigger, init) {
    var self = this, s = self.settings;
    var items = actionItems(ul);
    var dur = animate ? s.duration : 0;
    function clear() {
      if (trigger && trigger.classList) trigger.classList.remove(P + '_animating');
      if (parent) parent.classList.remove(P + '_animating');
    }
    if (ul.classList.contains(P + '_hidden')) {
      ul.classList.remove(P + '_hidden');
      if (!init) s.beforeOpen(trigger);
      slide(ul, true, dur, s.easingOpen, function () {
        clear();
        if (!init) s.afterOpen(trigger);
      });
      ul.setAttribute('aria-hidden', 'false');
      setAll(items, 'tabindex', '0');
      self._setVisAttr(ul, false);
    } else {
      ul.classList.add(P + '_hidden');
      if (!init) s.beforeClose(trigger);
      slide(ul, false, dur, s.easingClose, function () {
        ul.setAttribute('aria-hidden', 'true');
        setAll(items, 'tabindex', '-1');
        self._setVisAttr(ul, true);
        ul.style.display = 'none';
        clear();
        if (init) { if (trigger === 'init') s.init(); } else s.afterClose(trigger);
      });
    }
  };

  SlickNav.prototype._setVisAttr = function (ul, hidden) {
    var self = this;
    Array.prototype.forEach.call(ul.children, function (li) {
      if (li.tagName !== 'LI') return;
      children(li, 'UL').forEach(function (sub) {
        if (sub.classList.contains(P + '_hidden')) return;
        sub.setAttribute('aria-hidden', hidden ? 'true' : 'false');
        setAll(actionItems(sub), 'tabindex', hidden ? '-1' : '0');
        self._setVisAttr(sub, hidden);
      });
    });
  };

  SlickNav.prototype._outlines = function (show) {
    Array.prototype.forEach.call(document.querySelectorAll('.' + P + '_item, .' + P + '_btn'), function (el) {
      el.style.outline = show ? '' : 'none';
    });
  };

  SlickNav.prototype.toggle = function () { this._menuToggle(); };
  SlickNav.prototype.open = function () { if (this.btn.classList.contains(P + '_collapsed')) this._menuToggle(); };
  SlickNav.prototype.close = function () { if (this.btn.classList.contains(P + '_open')) this._menuToggle(); };

  /**
   * $(menu).slicknav(options) → UI.slicknav(menu, options).
   * $(menu).slicknav('open'|'close'|'toggle') → UI.slicknav(menu, 'open').
   */
  UI.slicknav = function (target, options) {
    var out = [];
    UI.toElements(target).forEach(function (el) {
      if (typeof options === 'string') {
        var inst = el._clSlicknav;
        if (inst && options.charAt(0) !== '_' && typeof inst[options] === 'function') inst[options]();
        if (inst) out.push(inst);
        return;
      }
      if (el._clSlicknav) { out.push(el._clSlicknav); return; }
      el._clSlicknav = new SlickNav(el, options || {});
      // Marked after cloning so the mobile copy does not inherit the flag.
      el.setAttribute('data-cl-slicknav', '1');
      out.push(el._clSlicknav);
    });
    return out;
  };
}());
...
/* ColorlibUI module: countdown — replaces The Final Countdown for jQuery 2.2.0.
 * Same date parsing ("2024/10/10", "2024-10-10 18:00", a timestamp or a Date),
 * the same offset fields and the same strftime directives (%D %H %M %S …, the
 * %-D no-padding and %!D:day,days; plural forms), checked every 100 ms and
 * reported once per second.
 *   $(el).countdown(date, function (event) { $(this).html(event.strftime(f)); })
 *   → ColorlibUI.countdown(el, date, function (event) { this.innerHTML = event.strftime(f); })
 * The callback runs with `this` = the element for the update, finish and
 * stoped events (as the plugin bound it to all three); the same events are
 * also dispatched on the element as 'update.countdown', 'finish.countdown'
 * and 'stoped.countdown' with the event data in event.detail. */
(function () {
  'use strict';
  var UI = window.ColorlibUI;
  if (!UI) return;
  ...
  var DIRECTIVES = {
    Y: 'years', m: 'months', n: 'daysToMonth', d: 'daysToWeek', w: 'weeks', W: 'weeksToMonth',
    H: 'hours', M: 'minutes', S: 'seconds', D: 'totalDays', I: 'totalHours', N: 'totalMinutes', T: 'totalSeconds'
  };
  var METHODS = { start: 1, stop: 1, toggle: 1, pause: 1, resume: 1, remove: 1 };

  function parseDate(value) {
    if (value instanceof Date) return value;
    if (String(value).match(MATCHERS)) {
      if (String(value).match(/^[0-9]*$/)) value = Number(value);
      // "2024-10-10" is read as UTC by Date(); with slashes it is local time, as intended.
  ...
  function pluralize(format, count) {
    ...
    if (format) {
      ...
      if (format.length === 1) plural = format[0];
      else { singular = format[0]; plural = format[1]; }
    }
  ...
  function strftime(offset) {
    return function (format) {
      var list = format.match(/%(-|!)?[A-Z]{1}(:[^;]+;)?/gi);
      if (list) {
        for (var i = 0; i < list.length; i++) {
          var m = list[i].match(/%(-|!)?([a-zA-Z]{1})(:[^;]+;)?/);
          var re = escapedRegExp(m[0]), modifier = m[1] || '', plural = m[3] || '', value = null;
          if (Object.prototype.hasOwnProperty.call(DIRECTIVES, m[2])) value = Number(offset[DIRECTIVES[m[2]]]);
          if (value !== null) {
            if (modifier === '!') value = pluralize(plural, value);
            if (modifier === '' && value < 10) value = '0' + value.toString();
            format = format.replace(re, value.toString());
          }
        }
      }
  ...
  function Countdown(el, finalDate, arg) {
    this.el = el;
    this.interval = null;
    this.offset = {};
    this.options = UI.extend({}, DEFAULTS);
    this.callback = null;
    if (typeof arg === 'function') this.callback = arg;
    else if (arg) this.options = UI.extend({}, DEFAULTS, arg);
  ...
  Countdown.prototype.start = function () {
    ...
    // Without a callback, listeners are attached right after this call returns
    // (the plugin's .countdown(date).on('update.countdown', fn) pattern): let them see the first tick.
    if (self.callback) self.update(); else setTimeout(function () { self.update(); }, 0);
    self.interval = setInterval(function () { self.update(); }, self.options.precision);
  };
  Countdown.prototype.stop = function () {
    clearInterval(this.interval);
    this.interval = null;
    this.dispatch('stoped');
  };
  Countdown.prototype.toggle = function () { if (this.interval) this.stop(); else this.start(); };
  Countdown.prototype.pause = function () { this.stop(); };
  Countdown.prototype.resume = function () { this.start(); };
  Countdown.prototype.remove = function () {
    this.stop();
    this.el._clCountdown = null;
    this.el.removeAttribute('data-cl-countdown');
  };
  Countdown.prototype.setFinalDate = function (value) { this.finalDate = parseDate(value); };

  Countdown.prototype.update = function () {
    // An element taken out of the page stops its own timer.
    if (!document.documentElement.contains(this.el)) { this.remove(); return; }
    var now = new Date();
    var left = Math.ceil((this.finalDate.getTime() - now.getTime()) / 1000);
    left = !this.options.elapse && left < 0 ? 0 : Math.abs(left);
    if (this.totalSecsLeft === left) return;
    this.totalSecsLeft = left;
    ...
    this.offset = {
      ...
      daysToWeek: Math.floor(days) % 7,
      daysToMonth: Math.floor(days % 30.4368),
      weeks: Math.floor(days / 7),
      weeksToMonth: Math.floor(days / 7) % 4,
      months: Math.floor(days / 30.4368),
      years: Math.abs(this.finalDate.getFullYear() - now.getFullYear()),
      totalDays: Math.floor(days),
      totalHours: Math.floor(left / 60 / 60),
      totalMinutes: Math.floor(left / 60),
      totalSeconds: left
    };
    if (this.options.elapse || left !== 0) this.dispatch('update');
    else { this.stop(); this.dispatch('finish'); }
  };

  Countdown.prototype.dispatch = function (type) {
    var detail = {
      type: type,
      namespace: 'countdown',
      finalDate: this.finalDate,
      elapsed: this.elapsed,
      offset: UI.extend({}, this.offset),
      strftime: strftime(this.offset)
    };
    if (this.callback) this.callback.call(this.el, detail);
    UI.emit(this.el, type + '.countdown', detail);
  };

  /**
   * ColorlibUI.countdown(el, date, callback|options) starts a countdown;
   * on an element that already has one, (el, 'stop'|'start'|'pause'|'resume'|'toggle'|'remove')
   * calls that method and (el, newDate) restarts it towards the new date.
   */
  UI.countdown = function (target, finalDate, arg) {
    var out = [];
    UI.toElements(target).forEach(function (el) {
      var inst = el._clCountdown;
      if (inst) {
        if (METHODS[finalDate]) inst[finalDate]();
        else if (finalDate !== undefined) { inst.setFinalDate(finalDate); inst.start(); }
      } else {
        inst = el._clCountdown = new Countdown(el, finalDate, arg);
        el.setAttribute('data-cl-countdown', '1');
      }
      out.push(inst);
    });
    return out;
