...
add_action( 'wp_enqueue_scripts', 'sunshine_sl_enqueue_scripts' );
function sunshine_sl_enqueue_scripts() {

	wp_enqueue_script( 'sunshine-post-likes', SUNSHINE_DIR_JS_URI . 'post-likes.js', array( 'sunshine-ui-js' ), '0.5-s2', true );
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_sunshine_process_simple_like', 'sunshine_process_simple_like' );
add_action( 'wp_ajax_sunshine_process_simple_like', 'sunshine_process_simple_like' );
function sunshine_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'sunshine' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'sunshine' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'sunshine' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'sunshine' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'sunshine' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'sunshine' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'sunshine_show_user_likes' );
add_action( 'edit_user_profile', 'sunshine_show_user_likes' );
function sunshine_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'sunshine' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'sunshine' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
