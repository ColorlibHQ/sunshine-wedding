...
function sunshine_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'sunshine' ),
        'next_string'     => esc_html__( '&raquo;', 'sunshine' ),
        'before_output'   => '<div class="sunshine-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'sunshine_pagination_defaults', $defaults )
    );
